...
def plot_test_accuracy(
    ...
    sparsity_levels = [round(sparsity_levels, 2) for sparsity_level in accuracy_lth.keys()]

    for sparsity_level, key in zip(sparsity_levels, accuracy_lth.keys()):
        c = next(colors)

        plt.plot(list(accuracy_lth[key].keys()), list(accuracy_lth[key].values()), '+-', label=f"LTH: {100 - sparsity_level:.2f}", c=c)
        plt.plot(list(accuracy_rand[key].keys()), list(accuracy_rand[key].values()), '+-', label=f"Rand: {100 - sparsity_level:.2f}", c=c)
